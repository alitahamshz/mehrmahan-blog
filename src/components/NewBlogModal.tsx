      ...
      {open &&
        createPortal(
          <div
            ...
            className={`fixed inset-0 z-9999 flex items-center justify-center p-4 transition-opacity duration-200 ${visible ? 'opacity-100' : 'pointer-events-none opacity-0'
              }`}
            style={{ backgroundColor: visible ? 'rgba(0,0,0,0.65)' : 'rgba(0,0,0,0)' }}
            onClick={handleClose}
          >
            <div
              className={`w-full max-w-180 rounded-2xl bg-bg p-8 transition-all duration-200 ${visible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
                }`}
              dir="rtl"
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={handleClose}
                    className="h-12 flex xs:w-full sm:flex-1  justify-center items-center border border-secondary flex-1 rounded-lg border border-neutral-gray-border bg-white font-bold text-text-default transition hover:bg-neutral-gray"
                  >
                    انصراف
                  </button>
                  <button
                    type="submit"
                    className="h-12 flex xs:w-full sm:flex-1 justify-center items-center rounded-lg bg-secondary font-bold text-white transition hover:opacity-90"
                  >
                    ثبت و انتشار بلاگ
                  </button>
